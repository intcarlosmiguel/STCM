...
static void free_CONSTANT(struct constant *CONSTANT,int categorias,bool is_undirect){
    int i;
...
static void init_MCE(struct constant *CONSTANT,struct mce *MCE,struct Graph *G,igraph_vector_t* faixas,bool is_undirect,int categorias){
    int i,j,k;
    double r;
...
static int* remove_element(int* vetor,int size,int i){
    int temp = vetor[i];
...
static void init_CONSTANT(struct constant *CONSTANT,bool is_undirect,int* category){
    int categorias;
    char filename[200];
    if(is_undirect) sprintf(filename,"./input/POLYMOD/faixas.txt");
    else sprintf(filename,"./input/polblogs/faixas.txt");
    categorias = size_txt(filename);
    ...
    for (i = 0; i < categorias; i++){
        ...
        for (j = 0; j < N; j++)
            fscanf(file, "%d %lf", &a, &CONSTANT->distribution_out[i][j]);

        CONSTANT->prob_out[i] = (double*) malloc(sizeof(double) * categorias);

        for (j = 0; j < categorias; j++)
            fscanf(arquivo, "%lf", &CONSTANT->prob_out[i][j]);

        fscanf(arquivo, "\n");
        fscanf(datei, "%lf", &CONSTANT->p_faixas[i]);
...
static void print_lig(struct mce *MCE, igraph_vector_t* faixas,int N){
    int i,j;
...
static double generate_conections(struct Graph *G,struct mce *MCE, igraph_vector_t* faixas,double p,igraph_t* Grafo,double total_edges){
    double ligacoes_total = 0;
    int i,j;
    for (i = 0; i < G->Nodes; i++) ligacoes_total += somatorio(MCE->degree_out,i,MCE->categorias);
    if(MCE->is_undirect) return (double)ligacoes_total/(2*total_edges);
    return (double)ligacoes_total/(total_edges);
}

static bool check_ligacao(struct Graph *G,int site,int vizinho){
    int i;
    for( i = 0;i < G->viz[site][0]; i++) if(vizinho == G->viz[site][i+1]) return true;
    return false; 
}

static struct Graph weighted_add_edge(struct Graph *G,double p,struct mce *MCE,int *n,int* site_per_faixas,int site,int faixa_vizinho,igraph_vector_t* faixas,igraph_vector_int_t* edges){
...
static void load_test(struct mce *MCE,igraph_vector_t* faixas,int N,struct Graph *G){
    ...
    char size_file[200];
    sprintf(size_file,"./input/test/faixas.txt");
    int n = size_txt(size_file);
    ...
    for (i = 0; i < N*n; i++) {
        ...
        if (i < n) {
            ...
            if (fscanf(datei, "%d", &a) != 1) {
                printf("Erro ao ler valor de 'a'\n");
            }
            VECTOR(*faixas)[i] = a;
        } 
        else {
...
static igraph_t stochastic_configuration_model(int N,bool is_undirect, double p,int seed,double* perca){
    ...
    int** n_faixas = (int**)calloc(categorias,sizeof(int*));
    int*** site_per_faixas = (int***)malloc(categorias*sizeof(int**));
    for ( i = 0; i < MCE.categorias; i++){
        n_faixas[i] = (int*) calloc(categorias,sizeof(int));
        site_per_faixas[i] = (int**) malloc(categorias*sizeof(int*));
        for ( j = 0; j < MCE.categorias; j++)site_per_faixas[i][j] = (int*) malloc(0*sizeof(int));
    }
    for(i = 0; i < G.Nodes; i++){
        f = VECTOR(faixas)[i];
        ...
        for(j = 0;j < MCE.categorias;j++){
            if(is_undirect){
                if(MCE.degree_out[i][j] != 0){
                    n_faixas[f][j]++;
                    site_per_faixas[f][j] = (int*)realloc(site_per_faixas[f][j],n_faixas[f][j]*sizeof(int));
    ...
    for (site = 0; site < N; site++){
        ...
        if(p > 0){
            int** n_faixas_vizinhos = (int**)calloc(categorias,sizeof(int*));
            int*** site_per_faixas_vizinhos = (int***)malloc(categorias*sizeof(int**));

            for ( i = 0; i < MCE.categorias; i++){
                n_faixas_vizinhos[i] = (int*)calloc(categorias,sizeof(int));
                site_per_faixas_vizinhos[i] = (int**)malloc(categorias*sizeof(int*));
                for ( j = 0; j < MCE.categorias; j++)site_per_faixas_vizinhos[i][j] = (int*) malloc(0*sizeof(int));
                ...
                for(j = 0;j < MCE.categorias;j++){
                    if(is_undirect){
                        if(MCE.degree_out[vizinho][j] != 0){
                            ...
                            site_per_faixas_vizinhos[f][j] = (int*)realloc(site_per_faixas_vizinhos[f][j],n_faixas_vizinhos[f][j]*sizeof(int));
    ...
    *perca = generate_conections(&G,&MCE,&faixas,p,&Grafo,total_edges);
    ...
    for (i = 0; i < G.Nodes; i++){
        ...
        if(!MCE.is_undirect){
            ...
            igraph_neighbors(&Grafo, &sucessors, i, IGRAPH_OUT,IGRAPH_NO_LOOPS,IGRAPH_NO_MULTIPLE);
            ...
            igraph_neighbors(&Grafo, &predecessors, i, IGRAPH_IN,IGRAPH_NO_LOOPS,IGRAPH_NO_MULTIPLE);
...
static double calcular_correlacao(igraph_vector_t* vetor1, igraph_vector_t* vetor2) {
...
static void calcula_propriedades(igraph_t *Grafo,double p,int N, double *resultados) {
    ...
    igraph_vector_scale(&graus,(double)1/N);
    ...
    double mediana = (N % 2 == 0)
        ? ((double)VECTOR(degrees)[mid - 1] + (double)VECTOR(degrees)[mid]) / 2.0
        : (double)VECTOR(degrees)[mid];


    // calculate the mean shortest path
    igraph_average_path_length(Grafo,NULL, &caminho_medio, NULL, IGRAPH_UNDIRECTED, 1);

    // Calcula o diâmetro da rede
    igraph_diameter(Grafo,NULL, &diametro, NULL, NULL, NULL, NULL, IGRAPH_UNDIRECTED, 1);
...
static void generate_stochastic_configuration_model(int N,bool is_undirect,double p, int redes,int seed){
    ...
    for (i = 0; i < redes; i++){
        ...
        igraph_t G;
        G = stochastic_configuration_model(N,is_undirect,p,seed+i,&perca);
