...
void init_MCE(struct constant *CONSTANT,struct mce *MCE,int* n_faixas,int** site_per_faixas,struct Graph *G,igraph_vector_t* faixas,bool is_undirect,int categorias){
    int i,j,k;
    double r;
    ...
    for (i = 0; i < G->Nodes; i++){
        ...
        n_faixas[faixas2[site]]++;
        site_per_faixas[faixas2[site]] = realloc(site_per_faixas[faixas2[site]],n_faixas[faixas2[site]]*sizeof(int));
        site_per_faixas[faixas2[site]][n_faixas[faixas2[site]] - 1] = i;
    }
    free(faixas2);
    free(grau_out);
...
struct Graph weighted_add_edge(struct Graph *G,double p,struct mce *MCE,int* n_faixas,int** site_per_faixas,int site,int faixa_vizinho,igraph_vector_t* faixas,igraph_vector_int_t* edges){

    int i,vizinho,faixa_site;
    faixa_site = (int) VECTOR(*faixas)[site];
    for ( i = 0; i < n_faixas[faixa_vizinho]; i++){
        if(MCE->degree_out[site][faixa_vizinho] == 0) break;

        vizinho = site_per_faixas[faixa_vizinho][i];
        ...
        if(genrand64_real1() <= p){
            ...
            if(MCE->is_undirect) MCE->degree_out[vizinho][faixa_site]--;
...
igraph_t local_configuration_model(int N,bool is_undirect, double p,int seed,double* perca){
    ...
    for (i = 0; i < MCE.categorias; i++) site_per_faixas[i] = (int*) malloc(0*sizeof(int));
    
    init_MCE(&CONSTANT,&MCE,n_faixas,site_per_faixas,&G,&faixas,is_undirect,categorias);
    ...
    for (site = 0; site < N; site++){
        for (faixa = 0; faixa < categorias; faixa++){
            ...
            G = weighted_add_edge(&G,1,&MCE,n_faixas,site_per_faixas,site,faixa,&faixas,&edges);
        }
        if(p > 0){
            int **vizinhos = (int**) malloc(categorias*sizeof(int*));
            int *q_vizinhos = (int*) calloc(categorias,sizeof(int));
            for (i = 0; i < categorias; i++) vizinhos[i] = (int*) malloc(0*sizeof(int));
            int n = 0;
            ...
            for (i = 0; i < G.viz[site][0]; i++){
                ...
                if(vizinho < 0) continue;
                if(somatorio(MCE.degree_out,vizinho,categorias) !=0){
            ...
            if(n > 1){
                ...
                for (i = 0; i < G.viz[site][0]; i++){
                    ...
                    for (faixa = 0; faixa < categorias; faixa++){
                        ...
                        G = weighted_add_edge(&G,p,&MCE,q_vizinhos,vizinhos,vizinho,faixa,&faixas,&edges);
                    }
                }
            }
            for(i = 0; i < categorias; i++) free(vizinhos[i]);
            free(vizinhos);
            free(q_vizinhos);
        }
    ...
    *perca = generate_conections(&G,&MCE,&faixas,p,&Grafo);
    ...
    free(G.viz);
    for(i = 0; i < categorias; i++) free(site_per_faixas[i]);
